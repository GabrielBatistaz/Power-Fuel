...
const ClientCheckout = () => {
    ...
    const [paymantSuccess, setPaymantSuccess] = useState(false);

    const router = useRouter();

    console.log("paymantIntent",paymentIntent);
    ...
    useEffect(() =>{
        if(Productsheets){
            ...
            fetch("/api/gerarPagamento",{
                method: 'POST',
                headers: {'Content-Type':'application/json'},
                body: JSON.stringify({
                    items: Productsheets,
                    intencaode_pagamento_id:paymentIntent
                })
            }).then((res) => {
                setLoading(false)
                if(res.status === 401){
